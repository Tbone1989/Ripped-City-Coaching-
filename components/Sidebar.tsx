...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, role, setRole, isOpen, setIsOpen, tier = 'BEGINNER' }) => {
  const menuItems = [
    ...
    { id: 'photos', label: 'Performance Timeline', icon: 'fa-images', roles: [UserRole.CLIENT, UserRole.COACH], minTier: 'BEGINNER' },
    ...
    { id: 'billing', label: 'Ledger', icon: 'fa-wallet', roles: [UserRole.COACH, UserRole.CLIENT], minTier: 'BEGINNER' },
  ...
  const filteredItems = menuItems.filter(item => {
    ...
    if (role === UserRole.COACH) return roleMatch;
    ...
    return roleMatch && athleteTierValue >= itemTierValue;
  });

  return (
    <div className={`
      ${isOpen ? 'w-64' : 'w-0 lg:w-20'} 
      transition-all duration-300 ease-in-out h-full glass border-r border-gray-800 flex flex-col z-20 overflow-hidden
    `}>
      ...
      <nav className="flex-1 py-6 overflow-hidden">
        <ul className="space-y-2 px-3 overflow-y-auto h-full scrollbar-hide">
          {filteredItems.map((item) => (
            <li key={item.id}>
              ...
              >
                ...
                {isOpen && <span className="text-sm font-bold uppercase tracking-wider">{item.label}</span>}
              </button>
            </li>
          ))}
        </ul>
      ...
      <div className="p-4 mt-auto border-t border-gray-800">
        <div className="flex flex-col gap-2">
          {isOpen && <span className="text-[10px] text-gray-500 font-bold uppercase mb-2">Role Management</span>}
          <div className="flex flex-col gap-2">
            {[UserRole.COACH, UserRole.CLIENT, UserRole.PROSPECT].map(r => (
              <button
                key={r}
                onClick={() => setRole(r)}
                className={`
